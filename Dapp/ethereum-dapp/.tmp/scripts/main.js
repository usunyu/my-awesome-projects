...
function doGetNodeStatus() {
    ...
    web3.net.getListening(function (error, result) {
        if (error) setData('get_peer_count', error, true);else {
...
function doGetCompilers() {
    ...
    web3.eth.getCompilers(function (error, result) {
        if (error) {
            ...
            if (result.length == 0) setData('list_of_compilers', 'No Compilers!!!', true);else setData('list_of_compilers', result, false);
        }
    });
}
...
function doDeployContractSynchronous() {
    ...
    web3.eth.getTransactionReceipt(transactionHash, function (error, result) {
        if (error) console.log('SENDTran Error=', error);else if (error) console.log('SENDTran Hash=', result);
    });

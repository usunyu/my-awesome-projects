...
public class RaycastPixel : MonoBehaviour
{
    ...
    void HandleMouseInput(int layerMask)
    {
        // One touch = paint a portal
        if (Input.GetMouseButton(0))
        {
            RaycastHit h;
            // First: does our touch intersect with an existing portal? If so, draw on that one instead of making a new one
            if (Physics.Raycast(cam.ScreenPointToRay(Input.mousePosition), out h, Mathf.Infinity, layerMask))
            {
                UpdatePixels(h);
            }
            else
            {
                ...
                if (canPlacePortal)
                {
                    ...
                    if (Physics.Raycast(cam.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity, wallLayer))
                    {
                        PlacePortalTest(hit);
                        canPlacePortal = false;
                    }
    ...
    void Update()
    {
        ...
        HandleTouchInput(layerMask);
        //HandleMouseInput(layerMask);
    }

    void PlacePortalTest(RaycastHit h)
    {
        //var anchor = Session.CreateAnchor(h.Point, Quaternion.identity);
        var placedObject = Instantiate(portal, h.point, Quaternion.identity, h.transform);
